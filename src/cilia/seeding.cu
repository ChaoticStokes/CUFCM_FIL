...
// #include "../../globals.hpp"
 ...
 void full_mcc_seeding(Real *const pos_ref, Real *const polar_dir_refs, Real *const azi_dir_refs, Real *const normal_refs, const int N, shape_fourier_description& shape, Real disc_r){
    ...
    int row_tot = 0;
    int column_tot = 0;
    double length_row = 3.75;
    double length_column = 24;
    int Tol = 5;
    ...
    double x_shift = (0.5 * column_tot + 1) *length_column;
    double y_shift = (0.5 * row_tot + 0.5) * length_row;
    double patch_spacing_x = 240;
    double patch_spacing_y = 240;
    ...
    std::uniform_real_distribution<> dis3(0, 1.0);
    // Use dis to transform the random unsigned int generated by gen into a 
    // double in [0, 1). Each call to dis(gen) generates a new random double.
    ...
    for (int patch_index = 0; patch_index < N_patch; patch_index++){
      ...
      double patch_center_x = patch_spacing_x * (column_patch - (N_patch_side / 2) + 0.5);
      double patch_center_y = patch_spacing_y * (row_patch - (N_patch_side / 2) + 0.5);
      if (N_patch_side % 2 != 0){
        patch_center_x = patch_center_x - 0.5 * patch_spacing_x;
        patch_center_y = patch_center_y - 0.5 * patch_spacing_y;
      }
      patch_phase[patch_index] = dis2(gen2) * 2 * PI;
      patch_orientation[patch_index] = dis3(gen3) * 0.852 - 0.426;
      ...
      for (int i = 0; i < 2 * row_tot + 1; i++){
        ...
        for (int j = 0; j < column; j++){
          double x_non_rot = (j + 1 + (column == column_tot) * 0.5) * length_column - x_shift;
          double y_non_rot = (i + 1) * length_row * 0.5 - y_shift;
          pos_ref[3*n] = patch_center_x + cos(patch_orientation[patch_index]) * x_non_rot + sin(patch_orientation[patch_index]) * y_non_rot;
          pos_ref[3*n + 1] = patch_center_y - sin(patch_orientation[patch_index]) * x_non_rot + cos(patch_orientation[patch_index]) * y_non_rot;
          pos_ref[3*n + 2] = 0;
  ...
  void seed_blobs(Real *const blob_references, Real *const polar_dir_refs, Real *const azi_dir_refs, Real *const normal_refs){
    ...
    shape_fourier_description shape;
      ...
      rod_seeding(blob_references, polar_dir_refs, azi_dir_refs, normal_refs, NBLOB, shape);
        ...
        Real disc_radius = 1750;//BLOB_SPACING*my_sqrt(NBLOB);
        // std::ifstream in("separation.dat"); // using the 6th number as input
        // for (int di=0; di<6; di++){
        //   in >> disc_radius;
        // }
